...
test.describe('UI - Verify the correct login functionality', ()=>{ 
  ...
  test('Login with both valid credentials',  { tags: ['@smoke', '@high'] }, async ({ page }) => {
  ...
  test('Login with  valid e-mail and invalid password',{ tags: [ '@low'] }, async ({ page }) => {
    await page.goto(`/`);
    await loginPage.confirmConditions();
    await navPage.goToLoginPage();
    await expect(page).toHaveURL(/accounts\.lidl\.com\/Account/);
    ...
    await page.waitForTimeout(1000); // Waits for 1 second (1000 milliseconds)
    await expect(page).toHaveURL(/.*accounts\.lidl\.com\/Account\//);
    await expect(mainPage.welcomeHeader01).not.toBeVisible();
    });

    test('Login with  valid password and invalid e-mail',{ tags: ['@low'] }, async ({ page }) => {
      ...
      test('Login with both invalid password and invalid e-mail', { tags: ['@smoke'] }, async ({ page }) => {
